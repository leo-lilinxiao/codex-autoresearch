...
Kept experiments stay in history. Failed experiments are rolled back with `git reset --hard HEAD~1` to keep history clean. The results log (`research-results.tsv`) records every experiment -- kept or discarded -- as the true audit trail.
